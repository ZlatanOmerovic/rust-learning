#[derive(Debug)]
struct Rectangle {
    width: u32,
    height: u32,
}

impl Rectangle {
    fn set(&mut self, width: u32, height: u32) {
        self.width = width;
        self.height = height;
    }

    fn lock(self) -> i32 {
        0
    }

    fn area(&self) -> u32 {
        self.width * self.height
    }

    fn width(&self) -> bool {
        self.width > 0
    }

    fn can_hold(&self, other: &Rectangle) -> bool {
        self.width > other.width && self.height > other.height
    }
}

impl Rectangle {
    fn square(size: u32) -> Self {
        Self {
            width: size,
            height: size,
        }
    }

    fn rectangle(size: u32) -> Self {
        Self {
            width: size,
            height: size / 2,
        }
    }
}

fn main() {
    let rect1 = Rectangle { width: 30, height: 50 };
    println!("The area of the rectangle is {} square pixels.", rect1.area());

    let mut rect2 = Rectangle { width: 0, height: 0 };
    // rect2.lock();
    rect2.set(50, 50);
    println!("The area of the rectangle is {} square pixels.", rect2.area());

    if rect2.width() {
        println!("The rectangle has a non-zero width. It is {}px wide.", rect2.width);
    }

    let rect1 = Rectangle {
        width: 30,
        height: 50,
    };
    let rect2 = Rectangle {
        width: 10,
        height: 40,
    };
    let rect3 = Rectangle {
        width: 60,
        height: 45,
    };

    println!("{:#?}", rect1);
    println!("{:#?}", rect2);
    println!("{:#?}", rect3);

    println!("Can rect1 hold rect2? {}", rect1.can_hold(&rect2));
    println!("Can rect1 hold rect3? {}", rect1.can_hold(&rect3));

    println!("Can rect2 hold rect1? {}", rect2.can_hold(&rect1));
    println!("Can rect2 hold rect3? {}", rect2.can_hold(&rect3));

    println!("Can rect3 hold rect1? {}", rect3.can_hold(&rect1));
    println!("Can rect3 hold rect2? {}", rect3.can_hold(&rect2));

    let square = Rectangle::square(30);
    let rectangle = Rectangle::rectangle(30);

    println!("{:#?}", square);
    println!("{:#?}", rectangle);
}
